...
exports.getAuctionStats = exports.getUpcomingSets = exports.createAuctionQueue = exports.createAuctionSets = exports.groupPlayersByAuctionSet = exports.classifyPlayer = exports.OFFICIAL_AUCTION_ORDER = exports.AUCTION_SET_METADATA = void 0;
// ============================================================================
// AUCTION SET DEFINITIONS & ORDER
// ============================================================================
exports.AUCTION_SET_METADATA = {
    // MARQUEE PLAYERS
    M1: {
        code: 'M1',
 ...
 */
exports.OFFICIAL_AUCTION_ORDER = [
    'M1', 'M2',
    'BA1', 'BA2',
    'WK1', 'WK2',
    'AL1', 'AL2',
...
const isMarqueePlayer = (player) => {
    ...
    if (player.starRating >= 5 && player.basePrice >= 150)
        return true;
    ...
    if (superstars.some(name => nameLower.includes(name)))
        return true;
    return false;
};
/**
 ...
 */
const bowlingStyleFromName = (playerName) => {
    ...
    if (fastIndicators.some(ind => nameLower.includes(ind)))
        return 'fast';
    ...
    if (spinIndicators.some(ind => nameLower.includes(ind)))
        return 'spin';
    return 'medium';
};
/**
 ...
 */
const groupPlayersByAuctionSet = (players) => {
    const groups = new Map();
    // Initialize all sets
    for (const code of exports.OFFICIAL_AUCTION_ORDER) {
        groups.set(code, []);
    }
    // Classify and group each player
    players.forEach(player => {
        const { set } = (0, exports.classifyPlayer)(player);
...
exports.groupPlayersByAuctionSet = groupPlayersByAuctionSet;
/**
 * Create structured auction sets from players
 */
const createAuctionSets = (players) => {
    const groups = (0, exports.groupPlayersByAuctionSet)(players);
    const sets = [];
    for (const code of exports.OFFICIAL_AUCTION_ORDER) {
        const playerIds = (groups.get(code) || []).map(p => p.id);
        const metadata = exports.AUCTION_SET_METADATA[code];
    ...
    if (remainingIds.length > 0) {
        const metadata = exports.AUCTION_SET_METADATA['ACC'];
        sets.push({
            code: 'ACC',
            title: metadata.title,
            description: metadata.description,
...
exports.createAuctionSets = createAuctionSets;
/**
 * Create auction queue (ordered player IDs across all sets)
 */
const createAuctionQueue = (sets) => {
    const queue = [];
 ...
 */
const getUpcomingSets = (currentSetCode, count = 3) => {
    const currentIndex = exports.OFFICIAL_AUCTION_ORDER.indexOf(currentSetCode);
    if (currentIndex === -1)
        return [];
    return exports.OFFICIAL_AUCTION_ORDER
        .slice(currentIndex + 1, currentIndex + 1 + count)
        .map(code => exports.AUCTION_SET_METADATA[code]);
};
exports.getUpcomingSets = getUpcomingSets;
/**
 * Get auction statistics
 */
const getAuctionStats = (sets) => {
    return {
...
exports.getAuctionStats = getAuctionStats;
