...
function belongsToSet(p, expectedPrefix, autoLogic) {
...
const SET_DEFINITIONS = [
    {
        ...
        filter: (p) => belongsToSet(p, 'M1', (p) => p.starRating === 5 && !p.isOverseas)
    },
    {
        setName: 'M2 — Marquee Players',
        filter: (p) => belongsToSet(p, 'M2', (p) => p.starRating === 5 && p.isOverseas)
    },
    {
        setName: 'BA1 — Capped Batters',
        filter: (p) => belongsToSet(p, 'BA1', (p) => p.isCapped && p.role === 'BAT' && (p.starRating === 4 || p.starRating === 3))
    },
    {
        setName: 'WK1 — Capped Wicketkeepers',
        filter: (p) => belongsToSet(p, 'WK1', (p) => p.isCapped && p.role === 'WK' && (p.starRating === 4 || p.starRating === 3))
    },
    {
        setName: 'BL1 — Capped Bowlers',
        filter: (p) => belongsToSet(p, 'BL1', (p) => p.isCapped && p.role === 'BOWL' && (p.starRating === 4 || p.starRating === 3))
    },
    {
        setName: 'AL1 — Capped All-Rounders',
        filter: (p) => belongsToSet(p, 'AL1', (p) => p.isCapped && p.role === 'AR' && p.starRating >= 4) // >= 4 to catch AL1 auto
    },
    {
        setName: 'BA2 — Capped Batters',
        filter: (p) => belongsToSet(p, 'BA2', (p) => p.isCapped && p.role === 'BAT' && p.starRating <= 2)
    },
    {
        setName: 'WK2 — Capped Wicketkeepers',
        filter: (p) => belongsToSet(p, 'WK2', (p) => p.isCapped && p.role === 'WK' && p.starRating <= 2)
    },
    {
        setName: 'BL2 — Capped Bowlers',
        filter: (p) => belongsToSet(p, 'BL2', (p) => p.isCapped && p.role === 'BOWL' && p.starRating <= 2)
    },
    {
        setName: 'AL2 — Capped All-Rounders',
        filter: (p) => belongsToSet(p, 'AL2', (p) => p.isCapped && p.role === 'AR' && p.starRating <= 3)
    },
    {
        setName: 'UBA1 — Uncapped Batters',
        filter: (p) => belongsToSet(p, 'UBA1', (p) => !p.isCapped && p.role === 'BAT')
    },
    {
        setName: 'UWK1 — Uncapped Wicketkeepers',
        filter: (p) => belongsToSet(p, 'UWK1', (p) => !p.isCapped && p.role === 'WK')
    },
    {
        setName: 'UBL1 — Uncapped Bowlers',
        filter: (p) => belongsToSet(p, 'UBL1', (p) => !p.isCapped && p.role === 'BOWL')
    },
    {
        setName: 'UAL1 — Uncapped All-Rounders',
        filter: (p) => belongsToSet(p, 'UAL1', (p) => !p.isCapped && p.role === 'AR')
    }
];
// ─── Display Sorting ───────────────────────────────────────────────────────────
function sortByStarAndPrice(a, b) {
...
function buildSets() {
    ...
    const unsold = allPlayers_1.ALL_PLAYERS
