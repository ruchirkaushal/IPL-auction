...
export const IMAGE_CACHE_VERSION = 'v2026.05.19.3';

export const CUSTOM_IMAGE_MAPPINGS: Record<string, string> = {
...
export function getPlayerImageUrl(
  ...
  let targetUrl = CUSTOM_IMAGE_MAPPINGS[playerId];

  // 2. Fallback to raw photo url if present (already uses IPLHeadshot2026 base)
  if (!targetUrl && rawPhotoUrl && rawPhotoUrl !== DEFAULT_FALLBACK_IMAGE && !rawPhotoUrl.includes('Default-Men')) {
    targetUrl = rawPhotoUrl;
  }

  // 3. Fallback to default Men's silhouette
  if (!targetUrl) {
    targetUrl = DEFAULT_FALLBACK_IMAGE;
  }

  // 4. URL Normalization & Cache Busting
  try {
    const urlObj = new URL(targetUrl);
    urlObj.searchParams.set('v', IMAGE_CACHE_VERSION);
    return urlObj.toString();
  } catch (e) {
    if (targetUrl.includes('?')) {
      return `${targetUrl}&v=${IMAGE_CACHE_VERSION}`;
    }
    return `${targetUrl}?v=${IMAGE_CACHE_VERSION}`;
  }
}
